...
# 1. Handler para Erros de Validação (Pydantic)
...
async def validation_exception_handler(request: Request, exc: RequestValidationError):
    ...
    for error in exc.errors():
        ...
        campo = error.get("loc")[-1] # Pega o nome do campo
        msg = error.get("msg")
...
# 3. Handler Genérico (Caindo aqui, é bug no código ou erro 500)
...
async def general_exception_handler(request: Request, exc: Exception):
    ...
    print(f"❌ ERRO CRÍTICO NÃO TRATADO: {exc}") 
